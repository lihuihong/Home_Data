...
public class RentHomeDao {
	 ...
	 * 根据编号查询权限地区
	 */
	public RentHome getRegionById(String id) throws SQLException {

		String sql = "select * from region where id = ?";
		return runner.query(DBUtils.getConnection(), sql, new BeanHandler<>(RentHome.class,processor),id);
	 ...
	 * @throws SQLException
	 */
	/*public void add(RentHome rent) throws SQLException {
	 ...
	 * @throws SQLException
	 */
//	public void update(RentHome region) throws SQLException {
...
//	}
	...
	public void delete(int id) throws SQLException {
		String sql = "delete from rent_home where id=?";
		runner.update(DBUtils.getConnection(), sql, id);
	}

	
	public List<RentHome> getRentHomes(HttpServletRequest request) throws SQLException {
		
		String sql = "select * from rent_home";
		String keyword = request.getParameter("keyword");
		if(null != keyword && !("".equals(keyword))){
			sql += " where region like '%" + keyword + "%'";
		}
		return runner.query(DBUtils.getConnection(), sql, new BeanListHandler<>(RentHome.class,processor));
	}

	public List<Map<String,Object>> getDataByRegion() {

		String sql = "select region,count(1) as count,ROUND(avg(price+0),0) as price from rent_home group by region;";
